...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CCarboncoinAddress(CScriptID(redeemScript)).ToString();
...
std::map<CTxDestination, std::vector<COutput>> CWallet::ListCoins() const
{
    ...
    // https://github.com/carboncoin/carboncoin/pull/10340, but that change has been
    // postponed until after https://github.com/carboncoin/carboncoin/pull/10244 to
...
bool CWallet::SetAddressBook(const CTxDestination& address, const std::string& strName, const std::string& strPurpose)
{
    ...
    if (!strPurpose.empty() && !CWalletDB(*dbw).WritePurpose(CCarboncoinAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(*dbw).WriteName(CCarboncoinAddress(address).ToString(), strName);
}
...
{
    {
        ...
        std::string strAddress = CCarboncoinAddress(address).ToString();
    ...
    CWalletDB(*dbw).ErasePurpose(CCarboncoinAddress(address).ToString());
    return CWalletDB(*dbw).EraseName(CCarboncoinAddress(address).ToString());
}
 ...
 * https://carboncointalk.org/?topic=54527, or
 * https://github.com/carboncoin/carboncoin/pull/1393.
 */
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    return CWalletDB(*dbw).WriteDestData(CCarboncoinAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    if (!mapAddressBook[dest].destdata.erase(key))
        return false;
    return CWalletDB(*dbw).EraseDestData(CCarboncoinAddress(dest).ToString(), key);
}
