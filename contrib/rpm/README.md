...
The RPM spec file provided here is for Carboncoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://carboncoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Carboncoin git repository](https://github.com/carboncoin/carboncoin) but are not part of
...
Currently only `Source10` is used, for the example `carboncoin.conf` file.
...
in the carboncoin git repository. At present only an SVG version of the Carboncoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the carboncoin
...
needed when 0.12.1 is released, a proper fix is already in the Carboncoin git
master branch.
...
idea to build Carboncoin Core against a static version of that library compiled
...
The `carboncoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `carboncoin-server` package. At this point, most RPM based
...
The `carboncoin-server` package should have SELinux support. How to properly do
...
Carboncoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/carboncoin/). He has been
packaging Carboncoin for Fedora at least since 2012.
...
4. I renamed the `carboncoin` package that contains the Qt GUI to `carboncoin-core` as
that appears to be how the general population refers to it, in contrast to
`carboncoin-xt` or `carboncoin-classic`. I wanted to make sure the general population
