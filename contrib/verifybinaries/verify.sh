...
###   carboncoincore.org and carboncoin.org and compares them.
...
WORKINGDIR="/tmp/carboncoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://carboncoincore.org"
HOST2="https://carboncoin.org"
BASEDIR="/bin/"
...
if [ $? -ne 0 ]; then
   echo "carboncoin.org failed to provide signature file, but carboncoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "carboncoin.org and carboncoincore.org signature files were not equal?"
